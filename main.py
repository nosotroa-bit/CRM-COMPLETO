...
def modulo_dashboard():
    ...
    # FILTRAR solo clientes ACTIVOS (igual que en CRM)
    ...
    # Fila 2: Métricas financieras y de rendimiento
    col1, col2, col3, col4 = st.columns(4)
    
    with col1:
        if not df_clientes.empty and 'MRR' in df_clientes.columns:
            mrr_total = df_clientes['MRR'].sum()
            st.metric("💰 MRR Mensual", f"{mrr_total:.0f}€", help="Monthly Recurring Revenue de clientes activos")
        else:
            st.metric("💰 MRR Mensual", "0€")
    
    with col2:
        # Facturación anual proyectada (MRR * 12)
        if not df_clientes.empty and 'MRR' in df_clientes.columns:
            arr = df_clientes['MRR'].sum() * 12
            st.metric("📈 ARR Proyectado", f"{arr:.0f}€", help="Annual Recurring Revenue (MRR × 12)")
        else:
            st.metric("📈 ARR Proyectado", "0€")
    
    with col3:
        # Tasa de conversión real (leads convertidos / total leads histórico)
        # Contamos cuántos leads tienen estado "Cliente" (ya convertidos)
        leads_convertidos = len(df_leads[df_leads['Estado Lead'] == 'Cliente']) if 'Estado Lead' in df_leads.columns and not df_leads.empty else 0
        if total_leads > 0:
            tasa_conv = (leads_convertidos / total_leads) * 100
            st.metric("🎯 Tasa Conversión", f"{tasa_conv:.1f}%", help="Leads convertidos a cliente / Total leads")
        else:
            st.metric("🎯 Tasa Conversión", "0.0%")
    
    with col4:
        if not df_clientes.empty and 'Satisfacción (1-5)' in df_clientes.columns:
            satisfaccion_media = df_clientes['Satisfacción (1-5)'].mean()
            st.metric("⭐ Satisfacción", f"{satisfaccion_media:.1f}/5", help="Media de satisfacción de clientes activos")
        else:
...
def mostrar_proximas_acciones():
    ...
    if acciones:
        ...
        with col1:
            # Por defecto: Solo Hoy, Próximas y Futuras (SIN vencidas)
            estados_disponibles = df_acciones['Estado'].unique()
            estados_default = [s for s in estados_disponibles if s in ["🟡 Hoy", "🟢 Próxima", "📅 Futura"]]
            filtro_estado = st.multiselect("Filtrar por Estado", 
                estados_disponibles,
                default=estados_default,
                help="Por defecto se ocultan las vencidas. Selecciónalas si quieres verlas.")
...
def mostrar_clientes_activos():
    ...
    # FILTRAR AUTOMÁTICAMENTE: Excluir clientes de Baja
    if 'Estado' in df_clientes_todos.columns:
        df_clientes = df_clientes_todos[df_clientes_todos['Estado'] != 'Baja'].copy()
    else:
        df_clientes = df_clientes_todos.copy()
    ...
    if df_clientes.empty:
        ...
        # Mostrar si hay clientes de baja
        if not df_clientes_todos.empty:
            bajas = len(df_clientes_todos[df_clientes_todos['Estado'] == 'Baja'])
            if bajas > 0:
                st.caption(f"ℹ️ Hay {bajas} cliente(s) de Baja (no se muestran aquí)")
        return
...
def mostrar_clientes_inactivos():
    """Gestión de clientes pausados y de baja"""
    st.subheader("⏸️ Clientes Pausados y de Baja")
    
    df_clientes_todos = utils.leer_excel(config.ARCHIVO_CRM, "CLIENTES_ACTIVOS")
    
    # FILTRAR: Solo Pausados y Baja
    if 'Estado' in df_clientes_todos.columns:
        df_clientes = df_clientes_todos[df_clientes_todos['Estado'].isin(['Pausado', 'Baja'])].copy()
    else:
        df_clientes = pd.DataFrame()
    
    if df_clientes.empty:
        st.info("✅ No hay clientes pausados o de baja.")
        return
    ...
    with col1:
        ...
        st.metric("Total Inactivos", total)
    
    with col2:
        pausados = len(df_clientes[df_clientes['Estado'] == 'Pausado'])
        st.metric("Pausados", pausados)
    
    with col3:
        bajas = len(df_clientes[df_clientes['Estado'] == 'Baja'])
        st.metric("Bajas", bajas)
    ...
    with col1:
        estados = df_clientes['Estado'].unique().tolist()
        filtro_estado = st.multiselect("Filtrar por Estado", estados, default=estados)
    
    with col2:
        buscar = st.text_input("🔍 Buscar cliente", key="buscar_inactivos")
    ...
    # Mostrar tabla
    if not df_filtrado.empty:
        st.caption(f"Mostrando {len(df_filtrado)} de {len(df_clientes)} clientes inactivos")
        ...
        for idx, row in df_filtrado.iterrows():
            with st.container():
                ...
                with col1:
                    # Icono según estado
                    icono = "⏸️" if row['Estado'] == 'Pausado' else "❌"
                    st.markdown(f"{icono} **{row['Nombre Comercial']}**")
                    if 'Ciudad' in row:
                        st.caption(f"📍 {row.get('Ciudad', 'N/A')} | {row.get('Tipo Local', 'N/A')}")
                ...
                with col3:
                    if 'Precio Mensual' in row:
                        precio = row.get('Precio Mensual', 0)
                        st.write(f"💰 {precio:.0f}€/mes")
                ...
                with col5:
                    if st.button("✏️", key=f"edit_inact_{row['ID']}", use_container_width=True, help="Editar"):
                        st.session_state.editando_cliente_inactivo = row['ID']
                        st.rerun()
                
                # Mostrar notas si hay
                if 'Notas' in row and row.get('Notas'):
                    st.caption(f"📝 {row.get('Notas')}")
                
                st.markdown("---")
    else:
        st.warning("No hay clientes que coincidan con los filtros")
    
    # Modal de edición (reutilizar la misma lógica que clientes activos)
    if st.session_state.get('editando_cliente_inactivo'):
        id_cliente = st.session_state.editando_cliente_inactivo
        cliente = df_clientes_todos[df_clientes_todos['ID'] == id_cliente].iloc[0]
        
        st.markdown("---")
        st.subheader(f"✏️ Editando: {cliente['Nombre Comercial']}")
        
        with st.form("form_editar_cliente_inactivo"):
            st.write(f"**Cliente ID: {id_cliente}**")
            
            col1, col2 = st.columns(2)
            
            with col1:
                nombre_comercial = st.text_input("Nombre Comercial*", value=cliente.get('Nombre Comercial', ''), key="edit_inact_nombre")
                servicio = st.selectbox("Servicio Contratado*",
                    ["Por definir", "Básico", "Premium", "Cuota Mensual", "Proyecto Puntual"],
                    index=["Por definir", "Básico", "Premium", "Cuota Mensual", "Proyecto Puntual"].index(cliente.get('Servicio Contratado', 'Por definir')) if cliente.get('Servicio Contratado') in ["Por definir", "Básico", "Premium", "Cuota Mensual", "Proyecto Puntual"] else 0,
                    key="edit_inact_servicio")
                precio_mensual = st.number_input("Precio Mensual (€)", 
                    min_value=0.0, value=float(cliente.get('Precio Mensual', 0)), step=50.0, format="%.2f", key="edit_inact_precio")
            
            with col2:
                estado = st.selectbox("Estado*",
                    ["Activo", "Pausado", "Baja"],
                    index=["Activo", "Pausado", "Baja"].index(cliente.get('Estado', 'Pausado')) if cliente.get('Estado') in ["Activo", "Pausado", "Baja"] else 1,
                    key="edit_inact_estado")
                satisfaccion = st.slider("Satisfacción (1-5)", 
                    min_value=1, max_value=5, value=int(cliente.get('Satisfacción (1-5)', 3)), key="edit_inact_satisfaccion")
            
            notas = st.text_area("Notas / Motivo de pausa o baja", value=cliente.get('Notas', ''), height=100, key="edit_inact_notas",
                               help="Documenta por qué el cliente está pausado o de baja")
            ...
            if submitted:
                if not nombre_comercial:
                    ...
                    df_actualizado = df_clientes_todos.copy()
                    ...
                    if utils.escribir_excel(config.ARCHIVO_CRM, "CLIENTES_ACTIVOS", df_actualizado):
                        st.success(f"✅ Cliente '{nombre_comercial}' actualizado")
                        
                        # Alertas según cambio de estado
                        if estado == "Activo" and cliente.get('Estado') != "Activo":
                            st.balloons()
                            st.success(f"🎉 ¡Cliente REACTIVADO! Ahora aparecerá en 'Clientes Activos'. MRR: {precio_mensual}€/mes")
                        elif estado != "Activo" and cliente.get('Estado') == "Activo":
                            st.warning(f"Cliente marcado como '{estado}'. MRR = 0€")
                        
                        del st.session_state.editando_cliente_inactivo
                        time.sleep(1.5)
                        st.rerun()
                    else:
...
def modulo_escandallos():
    ...
    # Cargar TODOS los clientes (para mantener datos)
    df_clientes_todos = utils.leer_excel(config.ARCHIVO_CRM, "CLIENTES_ACTIVOS")
